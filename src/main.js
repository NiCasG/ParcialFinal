...
function initSound() {
  ...
  var y = document.getElementById("endGame");
...
function initWorld() {
  ...
  const loader = new THREE.OBJLoader();
    ...
    mtlLoader.load('littleisle.mtl', function (materials) {
        ...
        loader.setPath('./modelos/island/');
        loader.load('littleisle.obj', function (Isle) {
              scene.add(Isle);
              Isle.scale.set(0.1, 0.1, 0.1);
              Isle.position.y = 0;
              Isle.position.x = 5;
...
function collisionAnimate() {
  ...
  var collisionResults = ray.intersectObjects(collectibleMeshList);
    if (collisionResults.length > 0 && collisionResults[0].distance < directionVector.length()) {
      ...
      document.getElementById("points").innerHTML = lives;//'toco, '+ JSON.stringify(collisionResults[0].object.name);//points;
      //camera.position.set(posX, posY, posZ);
      //MovingCube.position.set(posX, posY, posZ);
      //lives --;
      removeElement(JSON.stringify(collisionResults[0>=5].object.name));
      points++;
      ...
      document.getElementById("points").innerHTML = lives; // 'no toco';  
    }
}
